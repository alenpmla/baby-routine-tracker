...
- **Sleep tracking** — start/stop timer with live elapsed clock, today's list with each session's duration, and delete/edit entries. Sleeps are split into **Night** vs **Nap**: a sleep whose *start* falls in the local **7pm–9am** window counts as a **night sleep**, everything else as a nap. The label is auto-inferred from the start time and can be overridden in the sleep form when you add or edit a session. The Sleep screen shows exactly four stat tiles — **Total slept**, **Night sleep**, **Naps**, and **Avg/day** — and a **wake-window** line below the sleep-logging card showing how long the baby has been awake between sleep periods.
...
- **Settings** — edit the baby profile, manage the food-name suggestions, pick the **theme** (System, Light, or Dark), the **Home log** presentation (List or an M3 **Timeline** with natural-language wordings like "Woke up at 8:00 AM", "Had oats porridge + banana", "Had 180 ml bottle", "Wet diaper"), the **snapshot preferred units** and separate **PDF report units**, **export/import** a JSON backup, and **download a professional PDF report** for any date range (with a dedicated Feeding report and a per-day "Daily totals" breakdown showing each day's sleep, feeds, bottle & solids amounts, and diapers). The Report card offers **quick date-range presets** — This month, Last month, Past 3 months, Last 7 days, and Last 30 days — that fill the From/To dates for you. Before downloading, a dialog asks **which sections to include** — Summary, Daily totals, Sleep, Feeding, Diaper, Medication, Temperature, Weight, Head circumference, Teeth, Teething, and Milestones — with the last choice remembered. Sub-screens (Profile, Units, Data & reports, Food suggestions) open from the main Settings list.
...
- **Health tab** — under the Health tab (Home / Sleep / Feeding / Diaper / Health) you'll find **Weight**, **Head circumference**, **Teeth & teething**, **Milestones**, and **Medication & fever**. **Teeth**: log a tooth as it erupts (curated tooth pick) and see a mouth-grid chart of erupted teeth. **Teething**: log a day with symptom checkboxes (one entry per day, no future days) and a **Teething & sleep** card that compares average daily sleep and night-wakings on teething vs non-teething days over the configured window. **Milestones**: record developmental firsts (curated list or a custom label) and see a **Firsts** summary of the earliest date and age for each common milestone (roll over, sit up, crawl, first word, walk, etc.), plus a newest-first per-day list showing each entry's age at achievement with add/edit/delete/duplicate. **Medication & fever**: log a dose (name, optional amount + unit, note) or a temperature reading (value, °C/°F, optional location — rectal/axillary/ear/oral, note), quick or for a past time, with a latest-temperature tile and a per-day list supporting edit/delete/duplicate.
...
- `GET/POST /api/sleeps` · `DELETE /api/sleeps/:id` (same for `feedings`, `diapers`, `weights`, `headCircumferences`, `medications`, `temperatures`, `milestones`, `teeth`, `teethingDays`)
...
2. Use the bottom tab bar: **Home** (dashboard), **Sleep**, **Feeding**, **Diaper**, **Health** (weight, growth, milestones, medication & fever).
3. Sleep: tap *Start sleep timer*, then *Stop sleep*; the session lands in today's list. A sleep started between **7pm and 9am** local counts as **Night sleep**; otherwise it counts as a **Nap** — you can switch the label when adding or editing a sleep.
