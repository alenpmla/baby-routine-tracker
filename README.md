...
- **Sleep tracking** — start/stop timer with live elapsed clock, today's list with each session's duration, a **Total slept** stat tile for the day, and delete/edit entries. Sleeps are split into **Night** vs **Nap**: a sleep whose *start* falls in the local **7pm–9am** window counts as a **night sleep**, everything else as a nap. The label is auto-inferred from the start time and can be overridden in the sleep form when you add or edit a session; the day view shows separate **Night sleep** and **Naps** totals plus **wake-window lines** between naps.
